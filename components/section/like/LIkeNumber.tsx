          ...
          {inViewStates[key] && (
            <Reveal>
              <div className="bg-primary hover:translate-y-3 transition duration-300 w-52 h-52 flex flex-col items-center justify-center z-30 text-white pt-5 pb-5 rounded-xl">
                <h1 className="font-extrabold text-2xl text-center pb-1"> {/* Taille agrandie à text-4xl et centrée */}
                  <CountUp start={0} end={cardlist.number} duration={4} separator="" /> {/* Supprime les virgules */}
                  {cardlist.suffix}
                </h1>
                <p className="font-semibold text-center mt-2 text-xs"> {/* Taille du texte réduite à text-xs */}
